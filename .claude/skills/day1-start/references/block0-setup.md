...
| 항목 | 왜 필요한가 | 설치 방법 |
|------|-----------|----------|
| **Anthropic 계정** | Claude Code 로그인. 무료 플랜은 사용 불가 — Pro($20/월) 또는 Max($100/월) 구독 필요 | claude.ai에서 가입 + 구독 |
| **Git** | 프로젝트 다운로드 + Claude Code 내부 사용 | Mac: 터미널에 `git` 입력 → "개발자 도구 설치" 팝업에서 **설치** 클릭 / Windows: git-scm.com/downloads/win 에서 **Git for Windows** 설치 (필수) |
| **Node.js** | Claude Code 설치(npm) + Day 2 MCP 연결에 필요 | nodejs.org/ko/download → 설치 프로그램 다운로드 |
| **Cursor** | 파일을 보면서 작업하는 에디터 | cursor.com → Download → 설치 |

> 이미 README를 따라 설치했다면 아래로 넘어가세요.

### Claude Code 설치

Cursor에서 터미널을 열고 (`Terminal → New Terminal`), 아래 명령어를 **복사해서 붙여넣고** Enter:
...
| `git` 명령 안 됨 (Mac) | 터미널에 `git` 입력 → 팝업에서 "설치" 클릭 |
| `git` 명령 안 됨 (Windows) | git-scm.com/downloads/win 에서 Git for Windows 설치 |
...
1. **설치 확인**: 터미널에서 `git --version`, `node --version`, `claude --version` 각각 입력해서 동작하는지 확인
2. **안 되는 게 있으면**: 위 "필요한 것" 표를 참고해서 설치
3. **Claude Code 실행**: 터미널에서 `claude` 입력
4. **로그인**: 브라우저에서 Anthropic 계정 연결
5. **Output Style**: `/output-style` → Explanatory 선택
6. **첫 인사**: "안녕, 나는 [이름]이고 EXEM [팀명]에서 일해. 나한테 인사해줘"
